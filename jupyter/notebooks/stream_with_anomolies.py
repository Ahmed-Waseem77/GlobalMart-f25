...
ANOMALY_RATE = 0.05  # 5% chance of generating a business anomaly (fraud, stock error, etc.) 
...
# --- Null Injector Helper (Data Quality Issues) --- 
...
def generate_user(): 
    ...
    countries = ["Egypt", "Russia", "China", 
"Italy","Mongolia","Brazil", "Eswatini", "Timor-Leste", "Cambodia", 
"Federated States of Micronesia"] 
    weights = [110, 130, 500, 80,20, 213, 1.2, 1.3, 16, 0.1]  
    ...
    # [ANOMALY] Future Registration Date 
    if random.random() < ANOMALY_RATE: 
        # Anomaly: User registered in the future (Logical error) 
        reg_date = (datetime.now() + timedelta(days=365)).isoformat() 
        country_val = "Antarctica" # Anomaly: Unexpected Country 
    else: 
        reg_date = datetime.now().isoformat() 
        country_val = random.choices(countries, weights=weights, k=1)[0] 
...
def generate_product(): 
    ...
    # [ANOMALY] Inventory and Price Anomalies 
    if random.random() < ANOMALY_RATE: 
        anomaly_type = random.choice(['negative_price', 'negative_inventory', 'zero_stock']) 
        if anomaly_type == 'negative_price': 
            price = round(price * -1, 2) # Data Quality Anomaly 
        elif anomaly_type == 'negative_inventory': 
            inventory = random.randint(-100, -1) # Logical Anomaly 
        elif anomaly_type == 'zero_stock': 
            inventory = 0 # Low stock alert trigger 
...
def generate_transaction(): 
    ...
    country_preferences = { 
        ...
        "Eswatini": ["Home", "Books"], 
        "China": ["Books", "Beauty"], 
        "Brazil": ["Toys", "Clothing"], 
        "Italy": ["Home", "Sports"], 
        "Russia": ["Electronics", "Books", "Home"], 
        "Mongolia": ["Home", "Books", "Toys", "Sports", "Clothing"], 
    ...
    # [ANOMALY] Transaction Fraud/Outliers 
    ...
    for _ in range(num_products): 
        ...
        # Apply specific item anomalies 
        if is_anomaly: 
            # Anomaly: Abnormal Quantity (Bulk buying/Hoarding) 
            quantity = random.randint(50, 1000) 
            # Anomaly: Price Spike (Data error or extreme luxury item) 
            if price: 
                price = price * 10  
    ...
    # Calculate totals and inject anomalies into the payment 
    payment_method = random.choice(["Credit Card", "PayPal", "Debit Card", "Apple Pay", "Google Pay"]) 
     
    if is_anomaly and random.random() < 0.5: 
         payment_method = "Unknown_Method" # Anomaly: Invalid payment method 
...
def generate_session():
    """Generate a random session record with product interactions."""
    events = []
    ...
    # [ANOMALY] Bot Behavior / High Velocity
    if random.random() < ANOMALY_RATE:
        num_events = random.randint(50, 200) 
        
    for i in range(num_events):
        event_type = random.choice(["ADD_TO_CART", "REMOVE_FROM_CART", "CLEAR_CART", "CHECKOUT"])
        
        # Logic to attach a product_id only to relevant events
        prod_id = None
        if event_type in ["ADD_TO_CART", "REMOVE_FROM_CART"]:
            if PRODUCT_POOL:
                prod_id = random.choice(PRODUCT_POOL)["product_id"]
            else:
                prod_id = str(uuid.uuid4())
        
        events.append({
            "eventType": event_type,
            "product_id": prod_id,  # <--- NEW FIELD ADDED HERE
...
def main(): 
    ...
    print("Starting data stream with ANOMALIES enabled. Press Ctrl+C to stop.") 
